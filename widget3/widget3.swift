...
struct WidgetProvider: TimelineProvider {
    func placeholder(in context: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), quoteText: "There's so many lonely emojis man", backgroundColor: Color(hex: "c2cece"))
    }

    func getSnapshot(in context: Context, completion: @escaping (SimpleEntry) -> ()) {
        let entry = SimpleEntry(date: Date(), quoteText: "There's so many lonely emojis man", backgroundColor: Color(hex: "c2cece"))
        completion(entry)
    }
                ...
                URLSession.shared.dataTask(with: url) { data, response, error in
                    if let data = data, let quoteText = String(data: data, encoding: .utf8) {
                        ...
                        if quoteText.count < 50 {
                            ...
                            let entry = SimpleEntry(date: Date(), quoteText: quoteText, backgroundColor: Color(hex: "c2cece"))
...
struct WidgetBundle: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName("808s & Heartbreak")
        .description("I don't expect to be understood at all")
...
struct Widget_Previews: PreviewProvider {
    static var previews: some View {
        WidgetEntryView(entry: SimpleEntry(date: Date(), quoteText: "There's so many lonely emojis man", backgroundColor: Color(hex: "c2cece")))
