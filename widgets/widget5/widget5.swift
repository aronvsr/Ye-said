...
struct WidgetProvider: TimelineProvider {
    func placeholder(in context: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), quoteText: "Fuck whatever y'all been hearing", backgroundColor: Color(hex: "BDD2CF"))
    }

    func getSnapshot(in context: Context, completion: @escaping (SimpleEntry) -> ()) {
        let entry = SimpleEntry(date: Date(), quoteText: "Fuck whatever y'all been hearing", backgroundColor: Color(hex: "BDD2CF"))
        completion(entry)
    }
                ...
                URLSession.shared.dataTask(with: url) { data, response, error in
                    if let data = data, let quoteText = String(data: data, encoding: .utf8) {
                        ...
                        if quoteText.count < 50 {
                            ...
                            let entry = SimpleEntry(date: Date(), quoteText: quoteText, backgroundColor: Color(hex: "BDD2CF"))
        ...
        ZStack {
            ...
            VStack {
                Text(entry.quoteText)
                    ...
                    .padding()
                    .foregroundColor(Color(hex: "#271D1D"))
            }
        }
    }
}
...
struct WidgetBundle: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName("Yeezus")
...
struct Widget_Previews: PreviewProvider {
    static var previews: some View {
        WidgetEntryView(entry: SimpleEntry(date: Date(), quoteText: "Fuck whatever y'all been hearing", backgroundColor: Color(hex: "BDD2CF")))
