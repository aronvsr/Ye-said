...
struct WidgetProvider: TimelineProvider {
    func placeholder(in context: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), quoteText: "Winning is the only option", backgroundColor: Color(hex: "E40337"))
    }

    func getSnapshot(in context: Context, completion: @escaping (SimpleEntry) -> ()) {
        let entry = SimpleEntry(date: Date(), quoteText: "Winning is the only option", backgroundColor: Color(hex: "E40337"))
        completion(entry)
    }
                ...
                URLSession.shared.dataTask(with: url) { data, response, error in
                    if let data = data, let quoteText = String(data: data, encoding: .utf8) {
                        ...
                        if quoteText.count < 50 {
                            ...
                            let entry = SimpleEntry(date: Date(), quoteText: quoteText, backgroundColor: Color(hex: "E40337"))
          ...
          Image("BackgroundImage") // Use the correct image name
            ...
            VStack {
                Text(entry.quoteText)
                    ...
                    .padding()
                    .foregroundColor(Color(hex: "#F2F800"))
            }
        }
    }
}
...
struct WidgetBundle: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName("My Beautiful Dark Twisted Fantasy")
        .description("I was drinking earlier, now I'm driving")
...
struct Widget_Previews: PreviewProvider {
    static var previews: some View {
        WidgetEntryView(entry: SimpleEntry(date: Date(), quoteText: "Winning is the only option", backgroundColor: Color(hex: "E40337")))
