...
struct WidgetProvider: TimelineProvider {
    func placeholder(in context: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), quoteText: "I'm nice at ping pong", backgroundColor: Color(hex: "F78C58"))
    }

    func getSnapshot(in context: Context, completion: @escaping (SimpleEntry) -> ()) {
        let entry = SimpleEntry(date: Date(), quoteText: "I'm nice at ping pong", backgroundColor: Color(hex: "F78C58"))
        completion(entry)
    }
                ...
                URLSession.shared.dataTask(with: url) { data, response, error in
                    if let data = data, let quoteText = String(data: data, encoding: .utf8) {
                        ...
                        if quoteText.count < 50 {
                            ...
                            let entry = SimpleEntry(date: Date(), quoteText: quoteText, backgroundColor: Color(hex: "F78C58"))
...
struct WidgetBundle: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName("The Life of Pablo")
        .description("Yeezy with the big house, did it way different")
...
struct Widget_Previews: PreviewProvider {
    static var previews: some View {
        WidgetEntryView(entry: SimpleEntry(date: Date(), quoteText: "I'm nice at ping pong", backgroundColor: Color(hex: "F78C58")))
