...
struct WidgetProvider: TimelineProvider {
    func placeholder(in context: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), quoteText: "People only get jealous when they care.", backgroundColor: Color(hex: "F26B83"))
    }

    func getSnapshot(in context: Context, completion: @escaping (SimpleEntry) -> ()) {
        let entry = SimpleEntry(date: Date(), quoteText: "People only get jealous when they care.", backgroundColor: Color(hex: "F26B83"))
        completion(entry)
    }
                ...
                URLSession.shared.dataTask(with: url) { data, response, error in
                    if let data = data, let quoteText = String(data: data, encoding: .utf8) {
                        ...
                        if quoteText.count < 50 {
                            ...
                            let entry = SimpleEntry(date: Date(), quoteText: quoteText, backgroundColor: Color(hex: "F26B83"))
        ...
        ZStack {
            ...
            VStack {
                Text(entry.quoteText)
                    ...
                    .padding()
                    .foregroundColor(Color(hex: "#392900"))
            }
        }
    }
}
...
struct WidgetBundle: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        .configurationDisplayName("Watch the Throne")
        .description("Ball so hard, motherfuckers wanna fine me")
...
struct Widget_Previews: PreviewProvider {
    static var previews: some View {
        WidgetEntryView(entry: SimpleEntry(date: Date(), quoteText: "People only get jealous when they care.", backgroundColor: Color(hex: "F26B83")))
